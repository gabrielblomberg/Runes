...
public:
     ...
     */
    void add_entity(Entity entity, RenderFunction render);
...
public:
    ...
    private:
         ...
         */
        inline Lock(sf::RenderWindow *window, std::mutex *mutex)
            : m_window(window)
            , m_mutex(mutex)
        {
            m_mutex->lock();
     ...
     */
    inline Lock lock() {
        return Lock(m_window.get(), &m_mutex);
    }
